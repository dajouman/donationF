...
def load_data():
    ...
    # Pandas peut lire directement l'URL (plus propre)
    df = pd.read_csv(url)
    ...
    # LA CORRECTION MAJEURE : how='inner'
    # On ne garde QUE les parcelles du GeoJSON qui existent dans le Google Sheet.
    gdf_merged = gdf.merge(df, left_on='id', right_on='id_merge', how='inner')
    
    # Remplissage par défaut des textes pour le tableau
...
# Le zoom automatique fonctionne parfaitement car gdf ne contient plus que vos parcelles
if not gdf.empty:
    bounds = gdf.total_bounds
    m.fit_bounds([[bounds[1], bounds[0]], [bounds[3], bounds[2]]])
else:
    st.error("Aucune parcelle trouvée.")
...
for _, row in gdf.iterrows():
    ...
    # Formatage propre (les nombres sont garantis grâce à inner join + fillna)
    popup_text = f"<b>Parcelle :</b> {row.get('Parcelle', 'N/A')}<br>" \
                 f"<b>Contenance :</b> {row['contenance']:.4f} ha<br>" \
...
# Groupement simple (ne peut plus planter car les colonnes existent et sont numériques)

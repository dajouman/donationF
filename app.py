...
def load_data():
    ...
    # Sécurité : on vérifie que la colonne id_merge existe
    if 'id_merge' not in df.columns:
        st.error(f"Erreur : La colonne 'id_merge' est absente. Colonnes trouvées : {list(df.columns)}")
        st.stop()
